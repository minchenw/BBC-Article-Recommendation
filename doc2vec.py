...
def load_articles(articles_dirname, gloves):
    ...
    for path in allfiles:
        if path.endswith('.txt'):
            ...
            return_list.append((path.replace(articles_dirname+'/',''), contents[0].strip(), text, doc2vec(text, gloves)))
    return return_list
...
def doc2vec(text, gloves):
    ...
    for word in words_list:
        if word in gloves:
            word_vector_sum += gloves[word]
            count += 1
        else:
            pass
    return word_vector_sum/count
...
def distances(article, articles):
    ...
    for a in articles:
        if a != article:
            d = np.linalg.norm(a[3]-article[3])
            return_list.append((d,a))
    return return_list
...
def recommended(article, articles, n):
    ...
    """
    article_distances = sorted(distances(article,articles))[0:n]
    return [a[1] for a in article_distances]
